...
## [LRN-20260331-022] 根因统一分析：我在用"直觉"编程，不是"验证"编程

**10个bug统一根因：先实现，后验证，而不是先验证，后实现**

| bug | 我的假设 | 实际情况 |
|-----|---------|---------|
| SEO H1 | prompt生成H2 | API按字面生成H1 |
| DOTALL regex | `\\n`=真实换行 | `\\n`=两个字面符 |
| API路径 | 应该是/v4 | 实际是/v4/chat/completions |
| cron双触发 | flock可以防fork | flock在fork后各进程独立 |
| MEMDIR | append时变量还在 | append到文件末尾，不在内存 |
| PATH问题 | curl在PATH里 | cron环境PATH=/usr/bin |
| history类型 | dict | list |
| grep -v sh | 匹配shell进程 | 也匹配./clash |
| now_ts | sed只删注释 | 把定义也删了 |

**正确姿势（以后必须执行）：**
```
第一步：写代码前 → 先小规模验证假设（单独测试API路径、数据类型等）
第二步：写代码时 → 每写一个假设就注释这个假设是什么
第三步：写完代码 → 用"希望它work"以外的输入测试
第四步：部署前 → 确认"如果假设错了，什么会失败"
```
...
## [LRN-20260331-023] evolver进程活着≠Hub心跳正常

**症状：**
- evolver进程在跑（PID 491825）
- 但Hub最后一次心跳是13:26（8小时前）
- 进程是Node.js，console.log有缓冲，不是实时写的

**根因：**
- evolver的日志是append到文件，不是实时刷新
- Node.js的stdout有缓冲机制
- 没有强制flush时，日志可能延迟很久才写入

**修复：**
- evolver进程重启
- 或在重启命令中强制输出刷新

**自我验证原则：进程在跑≠功能正常，必须验证关键指标**
