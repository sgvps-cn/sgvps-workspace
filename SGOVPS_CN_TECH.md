...
## 二十、深化研究（2026-03-30 19:26）

### 定价机制
- 上游加价率统一为100（即成本=售价，无加价）—— 说明定价策略基于其他费用
- 价格范围：¥1（秒杀公益香港）至 ¥150（宁波高防）
- 83个在线产品，12个下架
...
| 客户 | 注册时间 | 订单数 | 活跃消费 |
|---|---|---|---|
| 刘海浪 | 2025-11-22 | 3 | ¥0（免费产品）|
| 马义彬 | 2025-12-05 | 1 | ¥1107/季度 |
| 3个未消费 | 2026-01 | 0 | ¥0 |

### CDN分析
- **字节火山引擎 ESA CDN**（via: ens-cache10.cn8519）
- 阿里云分析显示 IP属于字节跳动云计算
- 静态资源通过 ESA 加速
...
## 二十一、紧急问题发现（2026-03-30 19:33）

### 🚨 上游API故障（紧急）

**柠檬云 (nmvps.com)**
- 状态: DNS解析完全失败
- 错误: "Could not resolve host: www.nmvps.com"
- 影响: 12个产品无法同步
- 持续: 从活动日志可见，已失败数十次

**折云网络 (zheyunidc.cn)**
- 状态: API超时
- 错误: "CURL ERROR: Operation timed out"
- 影响: 83个产品无法同步
- 持续: 从活动日志可见，持续失败数小时
...
### 数据库健康
- 数据库大小: 34.89 MB
- 最大表: shd_activity_log (150,974条)
- 活跃主机: 2台，暂停: 1台


## 二十二、完整系统配置解析（2026-03-30 19:41）

### 公司信息
| 配置 | 值 |
|---|---|
| 公司名 | 星耀云 |
| ICP备案 | 粤ICP备2025497518号-1 |
| 客服QQ | 936380911 |
| 系统邮箱 | company@email.com |
| 客服邮箱 | sgvpsmall@163.com |
| 官网 | https://www.sgvps.cn |

### 系统版本
- WHMCS类系统: ZJMF (Multi-product billing platform)
- 当前版本: 3.7.6
- 系统版本类型: stable
- 最新检测版本: 3.7.6
- license: 6538AE275E9D3EA51F446A38CB1AB2AC
- zjmf_authorize: 75390Y8NB7V7cTefZol1Bk7ZijhUilU1qjNwhpGaGx6n9VBQKWysuLxHONkh

### 短信配置（⚠️ 未生效）
| 运营商 | 状态 | 凭证 |
|---|---|---|
| Nathansms | 配置但app_id=1 | 占位符 |
| Submail | 配置但app_id=1 | 占位符 |
| 阿里云短信 | 占位符 | 占位符 |
...
| 邮箱验证码登录 | 开启 |
| 微信登录 | 开启 |
| 二级验证 | 开启(支持重启/关闭等敏感操作) |

### Cron执行状态
| 任务 | 最后运行 | 状态 |
|---|---|---|
| Hourly Cron | 20分钟前 | ✅ 正常 |
| Daily Cron | 1.7天前 | ⚠️ 异常 |
| Halfhour Cron | 20分钟前 | ✅ 正常 |
...
/support.html — 帮助中心
/news.html — 新闻动态
/blog.sgvps.cn — 博客(已关闭)
```

### 登录主题定制
- 背景图: /upload/星耀云背景图.png
- 标题: 星耀云登录
- 副标题: 一站式开创新时代！
- 品牌标识: /upload/logo-colours.png


## 二十三、深度研究最终总结（2026-03-30 19:56）
...
| 短信服务 | ✅ 正常 | 阿里云SMS已配置 |
| 实名认证 | ✅ 正常 | 支付宝芝麻信用已配置 |
| SSL证书 | ✅ 有效 | 2026-11-11到期 |
| 数据库备份 | ✅ 存在 | 157MB, 2026-03-29 |
| 自动备份 | ✅ 运行 | BT-Panel每日备份 |
| FTP备份 | ✅ 配置 | 未启用本地备份 |
| 每日Cron | ⚠️ 延迟 | 约1.7天前运行 |
| Host ID 7删除循环 | ✅ 已修复 | overideautosuspend=1 |

### SSL证书问题
- 证书类型: 非Let's Encrypt/TrustAsia (付费证书)
- 无法通过BT-Panel自动续签
- 到期时间: 2026-11-11 (还有约7个月)
- 建议: 到期前1个月手动续签或切换到Let's Encrypt

### 库存同步状态
- 香港大浦三区-C: 本地9 vs 上游0 (不一致)
- 美国专线CN2 轻量云-B: 本地5 vs 上游6 (不一致)
- 其余产品同步正常

### 待处理项
1. SSL证书续签（2026-11到期）
2. 每日Cron执行延迟（BT-Panel cron已运行但ZJMF内部未更新）
3. Host ID 7手动彻底删除（已停止自动删除循环）
4. blog.sgvps.cn（已关闭）
